...
Сами CSS-строки значений (селекторы, размеры треков, отступы, media-запросы) **не** санитизируются — передача валидного CSS остаётся ответственностью вызывающей стороны.
...
- Библиотека генерирует только текст CSS — она не парсит, не валидирует и не экранирует произвольные CSS-значения.
